...
const RN_BACKEND = {
    ...
    if (!response.ok) {
        let message = `Backend HTTP ${response.status}`;
...
function buildLandslideFeatures(data, lat, lon) {
    const current = data.current || {};
    const daily = data.daily || {};

    const rain = (daily.rain_sum || []).map(Number);
    const precip = (daily.precipitation_sum || []).map(Number);
    const precipHours = (daily.precipitation_hours || []).map(Number);

    const clean = arr =>
        arr.map(v => Number.isFinite(v) ? v : 0);

    const rainValues = clean(rain);
    const precipValues = clean(precip);
    const hourValues = clean(precipHours);

    const sumFirst = (arr, count) =>
        arr.slice(0, count).reduce(
            (sum, value) => sum + value,
            0
        );
    ...
    return {
        rain_1d: rainValues[0] || 0,
        rain_3d: sumFirst(rainValues, 3),
        rain_7d: sumFirst(rainValues, 7),

        precip_1d: precipValues[0] || 0,
        precip_3d: sumFirst(precipValues, 3),
        precip_7d: sumFirst(precipValues, 7),

        precip_hours_1d: hourValues[0] || 0,
        precip_hours_3d: sumFirst(hourValues, 3),
        precip_hours_7d: sumFirst(hourValues, 7),
...
function renderPrediction(
    ...
    // -------------------------------------------------------
    // Determine the main displayed risk
    // -------------------------------------------------------
    ...
    // -------------------------------------------------------
    // Keep the existing hazard scores for comparison
    // -------------------------------------------------------
...
async function analyzePlace(place, hazard, target) {
    target.innerHTML = `
        <div class="rn-muted">
            Fetching live weather and running AI model
            for ${esc(place.name)}…
        </div>
    `;

    try {
        // 1. Get live + recent weather
        const raw = await getWeather(
            place.lat,
            place.lon
        );

        // 2. Existing weather summary
        const summary = summarizeWeather(raw);

        // 3. Run real ML model for landslide/auto mode
        let mlResult = null;
        ...
        if (
            ...
            mlResult = await predictLandslideML(
                raw,
                place.lat,
                place.lon
            );
        ...
        // 6. Save latest risk for emergency priority
        localStorage.setItem(
            "rakshanet_last_risk",
            String(risk.score || 0)
        );
        ...
        target.innerHTML = `
            <div class="rn-alert-banner danger">
...
function initEmergencyPage() {
    ...
    form.addEventListener(
        ...
        async event => {
            ...
            // -------------------------------------------------
            // Save only lightweight risk data locally
            // -------------------------------------------------
...
async function initMap(){
    ...
    function showLocationSearchLoading() {
        ...
        suggestionsBox.innerHTML = `
            ...
            >
                <span class="rn-location-result-title">
                    🔎 Searching locations...
                </span>

                <span class="rn-location-result-address">
...
async function selectMapLocation(item) {
    ...
    try {
        ...
        summary.innerHTML = `
            ...
            >
                AI result is a preliminary risk estimate
                and is not an official disaster warning.
        ...
        summary.innerHTML = `
            ...
            <span class="rn-alert-banner danger">
                Unable to load live location intelligence.
            </span>
...
function getActiveMapLocation() {
    ...
    // -----------------------------------------------------
    // 1. Prefer the location selected through map search
    // -----------------------------------------------------
    ...
    // -----------------------------------------------------
    // 2. Otherwise use saved GPS location
    // -----------------------------------------------------

    const gps =
        RakshaLocation.getSavedLocation();
    ...
    // -----------------------------------------------------
    // 3. Final fallback: current map center
    // -----------------------------------------------------
...
async function loadFacilities(
    ...
    // Show exactly where we are searching.
    if (searchStatus) {
    ...
    // Clear old facilities from this layer.
    layer.clearLayers();


    // Search around SELECTED LOCATION,
    // not automatically around GPS.
    const items =
        await searchNearby(
            location.latitude,
            location.longitude,
    ...
    items.forEach(item => {
        ...
        // Save as possible route destination.
    ...
    // Inform the user about the search area.
    if (searchStatus) {
    ...
    () => {
        ...
        RakshaLocation.getLocation(
            async loc => {
                ...
                // Make My Location the active
                // location for nearby resources.
                localStorage.setItem(
                    "rakshanet_map_location",
                    JSON.stringify({
                        latitude:
